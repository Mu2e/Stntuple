...
* handling MC truth                                                          
- for now , it is assumed that run numbers < 100,000 correspond to MC
  and run numbers >= 100,000 correspond to the real data
- when [[file:../mod/InitStrawHitBlock.cc::/rn_number < 100000/][InitStrawHitBlock]] is storing TStrawHit's, to save space,
  for real data the MC truth is not written into the ntuple
...
** TModule                                                                   
*** debugBits : one can set up to 100 individual debug bits per module.      
- Syntax:
#+begin_src
    debugBits : { 
	bit0:1  
...
** InitStntuple: initializations, fills all data blocks                      
- Stntuple data blocks are filled from [[file:../mod/InitStntuple_module.cc::/Event()->Init/]]
** StntupleMaker                                                             
...
** FillStntuple: end of event processing, resolves the cross-block links     
- [[file:../mod/FillStntuple_module.cc::/block->ResolveLinks]]
...
* ------------------------------------------------------------------------------
* maintenance - nightly builds etc                                           
- nightly build logs : file:/exp/mu2e/app/users/mu2epro/nightly2/secondary/repo
* ------------------------------------------------------------------------------
